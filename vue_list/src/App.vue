...
<template>
  <ul>
    <li v-for="entry in entries" :key="entry.node.postId">
      <p>{{ entry.node.title }}</p>
      <p>{{ entry.node.excerpt.replace(/(<([^>]+)>)/gi, "") }}</p>
      <p>{{ entry.node.content.replace(/(<([^>]+)>)/gi, "") }}</p>
      <p>
        <time datetime="{entry.node.date}">{{
          entry.node.date.substring(0, 4)
        }}</time>
      </p>
    </li>
  </ul>
